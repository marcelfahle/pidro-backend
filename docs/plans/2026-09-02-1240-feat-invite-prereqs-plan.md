...
- **Objective:** an invite link and a guest account can be added to Pidro without opening the API to abuse: callers are throttled by real client address, room codes cannot collide silently or be predicted (brute-force resistance belongs to the phase-1 invite code, not the four-character room code), the guest flag cannot be forged, a user's tokens can be revoked, and the domain-association files the app links need are served by the API.
...
- **Approach:**
  ...
  3. Deployment doc: the header contract, `TRUST_PROXY_HEADERS`, what happens if `forward_headers` is enabled, and a one-time production check to run after the first deploy: set `RATE_LIMIT_LOGIN_LIMIT=1` temporarily, confirm 429 from the operator's network and a non-429 answer from a second network (401 when the probe uses invalid credentials; the check proves bucket isolation, not authentication), then remove the override. A smoke probe cannot verify this without echoing the derived address, so the smoke test only gains a comment that limited routes must never join its path.
