...
- R8. Every successful first-time redeem inserts one `invite_redemptions` row (`invite_id`, `user_id`, `position`, `platform`, `source`) and increments `redeem_count`; the `(invite_id, user_id)` pair is unique so overlapping retries write the row, count and `seat_claimed` event exactly once. `invite_events` also receives `created`, `revoked`, `guest_created`, `guest_upgraded` and `game_completed` rows with `platform` and `ua_class` when known. `kind` is validated against the origin's kind list so later phases add kinds without a migration.
...
- R17. A guest reaper runs on a configurable interval, deletes guests whose `last_seen_at` (or `inserted_at` when nil) is older than 30 days with the R15 recipe, is disabled in the test environment, and exposes a synchronous run for tests. Its deletion transaction locks and reloads the user row, then deletes only if it is still a guest, so a concurrently completed upgrade is preserved.
...
- Case-insensitive username uniqueness; v1 deliberately keeps exact username matching. Email uniqueness is enforced case-insensitively in this phase (KTD7).
...
- KTD3. **Redeem is validate → claim → record, and the seat wins over the ledger.** The controller derives the state (R3), then makes one `RoomManager.claim_seat/4` call carrying `{room_id, user_id, hint, explicit_position}`; the GenServer checks lock/kick state, resolves `partner`, validates the target assignment, only then evicts a held seat elsewhere, fills the target seat and broadcasts `invite_redeemed`. Thus a failed target claim preserves an old held seat. The controller then calls `Invites.record_claim/2`, which serializes one `(invite_id, user_id)` with a transaction advisory lock and unique index and writes the redemption, `redeem_count` increment and `seat_claimed` event exactly once. A failed ledger write is logged and does not unseat the player, mirroring `persist_completed_game/5`. Chosen over recording first: an inflated count is worse than a missing funnel row. Governs R5, R8, R9.
...
- KTD7. **Upgrade runs `User.upgrade_changeset/2` and `increment_token_version/2` in one `Repo.transaction/1`, mints the token from the returned row, and does not broadcast a disconnect.** (session-settled: user-approved — chosen over a sessions table now: smallest change that revokes old tokens.) Uniqueness is pre-checked with `lower(email)` and exact username; a named unique `lower(email)` expression index closes concurrent registration/upgrade races, and both changesets map that constraint back to the email field so the API still answers 409. The migration aborts with a remediation message rather than choosing between pre-existing case-colliding identities. Governs R13.
- KTD8. **`Auth.delete_user/1` owns the deletion recipe used by `DELETE /auth/me` and the admin LiveViews; `Auth.delete_guest/1` applies it conditionally for the reaper:** one database transaction runs `Invites.revoke_hosted/1` (which also nulls `label`, the only personal data an invite tombstone carries), deletes `player_profiles`, `player_achievements`, `invite_redemptions`, `invite_events` and the user, then post-commit cleanup calls `RoomManager.leave_room/1`, recomputes invite windows and broadcasts disconnect. The guest-specific entry point first locks and reloads the user in that transaction and proceeds only while `guest: true`, so it cannot delete a concurrently upgraded account. This ordering keeps the account recoverable if persistent deletion fails; post-commit in-memory failures are logged. `PidroServer.Accounts.GuestReaper` is a self-scheduling GenServer (`enabled`, `interval_ms`, `max_idle_days` under `config :pidro_server, PidroServer.Accounts.GuestReaper`), started after the Repo, not scheduled when disabled, with `run_once/1`. Instantiates KD9; governs R15, R17.
...
- **Approach:**
  1. Migration: `invites` (binary id pk; `code` string unique; `room_id` uuid; `room_code` string; `host_user_id` uuid indexed; `seat_hint`, `label`; `redeem_count` default 0; `expires_at`, `revoked_at` utc_datetime_usec; `superseded_by` references `invites`; `timestamps(type: :utc_datetime_usec)`), `invite_redemptions` (`invite_id` references `invites` on delete cascade, `(invite_id, user_id)` unique, `user_id` indexed, `position`, `platform`, `source`, insert-only timestamps), `invite_events` (`invite_id`, `kind`, `platform`, `ua_class`, `user_id` nullable indexed, insert-only timestamps). No foreign keys to `users`, matching `20260607020000_create_player_achievements.exs`.
...
- **Test scenarios:**
  ...
  - `record_redemption/2` writes and counts one row per invite/user pair; concurrent `record_claim/2` calls write one redemption, count increment and `seat_claimed` event; `record_event/2` rejects an unknown kind.
...
- **Test scenarios:**
  ...
  - Concurrent case-variant upgrades leave one login identity and map the losing unique constraint to `:email_taken`; a reaper deletion waiting behind an upgrade preserves the now-registered account.
...
| Migration round trip | `mix ecto.rollback --step 3` then `mix ecto.migrate` in `apps/pidro_server` | U1 | both succeed |
...
- Every requirement R1–R28 has a passing test; every acceptance example AE1–AE15 is covered by at least one named test.
