...
- R7. `POST /api/v1/invites/:code/regenerate` revokes the old invite, mints a new one with the same hint and label, sets `superseded_by` on the old row, and answers 201 with the new invite. A revoked-and-superseded invite reads `revoked`, never `moved` (KD8).
...
- R15. `DELETE /api/v1/auth/me` leaves the caller's room, revokes invites the caller hosts and clears their labels, deletes the caller's `player_profiles`, `player_achievements`, `invite_redemptions` and `invite_events` rows and the `users` row in one transaction, broadcasts the socket disconnect, and answers 204 (KD9). `game_stats` and `abandonment_events` rows are untouched.
...
- KTD8. **`Auth.delete_user/1` becomes the single deletion recipe, shared by `DELETE /auth/me`, the reaper and the admin LiveViews:** `RoomManager.leave_room/1` (tolerating `:not_in_room`), `Invites.revoke_hosted/1` (which also nulls `label`, the only personal data an invite tombstone carries), then one transaction deleting `player_profiles`, `player_achievements`, `invite_redemptions`, `invite_events` and the user, then the disconnect broadcast. `PidroServer.Accounts.GuestReaper` is a self-scheduling GenServer (`enabled`, `interval_ms`, `max_idle_days` under `config :pidro_server, PidroServer.Accounts.GuestReaper`), started after the Repo, not scheduled when disabled, with `run_once/0`. Instantiates KD9; governs R15, R17.
...
- **Dependencies:** none.
- **Files:** `apps/pidro_server/priv/repo/migrations/<timestamp>_create_invites.exs` (create; three tables), `apps/pidro_server/priv/repo/migrations/<timestamp>_add_guest_reaper_index_to_users.exs` (create; partial index on `last_seen_at` where `guest`), `apps/pidro_server/lib/pidro_server/invites/invite.ex`, `invites/redemption.ex`, `invites/event.ex`, `invites/codes.ex`, `apps/pidro_server/lib/pidro_server/invites.ex` (create), `config/config.exs`, `config/dev.exs`, `config/test.exs`, `config/runtime.exs` (`link_base_url`), tests: `apps/pidro_server/test/pidro_server/invites/codes_test.exs`, `test/pidro_server/invites_test.exs` (create).
- **Approach:**
...
- **Test scenarios:**
  ...
  - With the generator forced to return a taken code once, `create_invite/1` retries and succeeds; forced twice, it returns the constraint error.
  ...
  - `regenerate/1` revokes the old row, sets `superseded_by`, and the old row's state is `revoked` even though the new room is waiting (KD8).
...
- **Goal:** the RoomManager can claim a seat from an invite with hint fallback, track lock, kick list and invite liveness, hide invited rooms from the lobby, and let the host move, lock and kick.
...
- **Approach:**
  ...
  3. `claim_seat/4` public API and `:claim_seat` callback, in this order: fetch by code; compare `room.id` with the given `room_id` (mismatch → `:room_not_found`); already seated at this table → `{:ok, room, position, true}` without changes (a held seat is left for the R20 channel reclaim); `ensure_room_joinable/1` (now also `:table_locked` and `:kicked`); `maybe_evict_disconnected_player/2` (a caller tracked in another room only); `ensure_not_in_other_room/3`; resolve `partner` through `partner_position/1` on the host's current seat (`:room_not_found` when the host has no seat); then `assign_with_fallback/3` or `assign/3` for an explicit position (a taken seat returns `{:error, {:seat_taken, Positions.available(room)}}`), `Seat.fill_seat/2`, ready check, `put_room_and_player/3`, broadcasts (`broadcast_room`, lobby, and `{:invite_redeemed, …}` carrying the `display_name` passed by the caller), then the existing auto-start.
...
- **Approach:**
  ...
  5. `GuestReaper` GenServer: `init` schedules only when `enabled`; `handle_info(:reap)` calls `run_once/0` and reschedules; `run_once/0` selects guest ids by the R17 predicate in batches of 100 and calls `delete_user/1` for each. Test config `enabled: false`; production default `enabled: true`, `interval_ms` one hour, `max_idle_days` 30.
...
- **Test scenarios:**
  ...
  - `GuestReaper.run_once/0` deletes a guest 31 days idle (and one with nil `last_seen_at` inserted 31 days ago), keeps a guest 29 days idle and a registered user idle for a year. Covers AE14.
