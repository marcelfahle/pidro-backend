defmodule Pidro.Bot.Knowledge do
  ...
  @doc """
  Returns the fourteen trumps of `suit`, highest first.
  ...
  def all_trumps(suit) do
    high = for rank <- 14..6//-1, do: {rank, suit}
    low = for rank <- 4..2//-1, do: {rank, suit}
    high ++ [{5, suit}, {5, Card.same_color_suit(suit)}] ++ low
  end
  ...
  def my_trumps(%SeatView{position: position, state: state}) do
    state.players[position].hand
    |> Enum.filter(&Card.is_trump?(&1, state.trump_suit))
    |> sort_ascending(state.trump_suit)
  end
  ...
  def played_cards(%SeatView{state: state}) do
    tricks = state.tricks ++ List.wrap(state.current_trick)
  ...
  def unseen_trumps(%SeatView{} = view) do
    known =
      MapSet.new(
        view.state.players[view.position].hand ++
          played_cards(view) ++ Enum.concat(Map.values(view.killed_cards))
      )

    view.state.trump_suit
    |> all_trumps()
  ...
  def current_winner(%SeatView{state: %{current_trick: %Trick{plays: [_ | _]} = trick} = state}) do
    {:ok, winner, _points} = Play.determine_trick_winner(trick, state.trump_suit)
  ...
  def beatable_by?(%SeatView{} = view, card, seats) do
    ...
    Enum.any?(seats, &(Map.get(view.hand_counts, &1, 0) > 0)) and
  ...
  def wins_trick?(%SeatView{} = view, card) do
    case current_winner(view) do
      {_winner, winning} -> beats?(card, winning, view.state.trump_suit)
      nil -> true
    end
  end
  ...
  def points_on_trick(%SeatView{state: %{current_trick: %Trick{plays: [_ | _]} = trick} = state}) do
    Scorer.score_trick(trick, state.trump_suit).winner_points
  end
  ...
  def opponent_five_on_trick?(%SeatView{} = view) do
    ...
    Enum.any?(trick_plays(view), fn {pos, card} ->
      Types.position_to_team(pos) != my_team and five?(card, view.state.trump_suit)
    end)
  end
  ...
  def side_points(%SeatView{state: state}) do
    state.tricks
    |> Enum.map(&Scorer.score_trick(&1, state.trump_suit))
  ...
  def bidding_side?(%SeatView{} = view), do: view.state.bidding_team == my_team(view)
  ...
  def opening_trick?(%SeatView{state: state}), do: state.tricks == []
  ...
  defp trick_plays(%SeatView{state: %{current_trick: %Trick{plays: plays}}}), do: plays
  defp trick_plays(%SeatView{}), do: []

  defp other_active_seats(%SeatView{position: position, state: state}) do
