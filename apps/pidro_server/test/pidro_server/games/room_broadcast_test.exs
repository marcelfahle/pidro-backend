defmodule PidroServer.Games.RoomBroadcastTest do
  ...
  test "broadcasting room update when player leaves", %{socket: _socket} do
    ...
    {:ok, _} = RoomManager.join_room(room_code, p3.id)
    {:ok, _} = RoomManager.join_room(room_code, p4.id)
    ...
    assert room.status == :playing # Game starts when 4 players join
    ...
    assert length(room_after_leave.player_ids) == 3
