...
- Body (optional): why and what (not how). Wrap at ~72 chars. Reference issues with `Refs #123` or `Closes #123`.
- Breaking changes: use `type!` or a `BREAKING CHANGE:` paragraph in the body
...
- Small, focused PRs; prefer several small over one large
...
- Checklist before review: `mix format --check-formatted`, `mix compile --warnings-as-errors`, `mix test`, `mix dialyzer`, `mix credo --strict`
- Wait for CI green; address feedback with clear commits; prefer squash-merge using a Conventional Commit title
